...
public static class mgame_manager
{
    ...
    public async static Task<bool> move_to_screen(string player_name_, string level_name, string game_key, string current_user_key, int pic_id) // move player to screen in particular game
    {
        ...
        try
        {
            ...
            await firebase_comm.put_object_into_path(new mgame_player { lvl = level_name, player_name = player_name_, playerkey = current_user_key, gamekey = game_key, playerpic=pic_id }, players_in_game_path); // assign player to lvl and make a record
    ...
    private static mgame create_new_game(string current_user_key)
    {
        mgame g = new mgame();
    ...
    public async static Task<mgame> get_or_start_new_multiplayer_game(string current_user_key) // we get available game, if its outdated or not exists we create new
    {
        mgame g = new mgame();
        ...
        try
        {
            ...
            else
            {
                res.ForEach(async x =>
                {
                    if ((x.obj.game_start.AddMinutes(15) < DateTime.UtcNow) || (x.obj.created_by_player_key == current_user_key)) // delete all outdated and curent game
                    {
                        await firebase_comm.delete_object_from_path_key(gamepath, x.key); //delete outdated game                                                                 //res.Remove(x);
                    }
                });

                res.RemoveAll(x => (x.obj.game_start.AddMinutes(15) < DateTime.UtcNow) || (x.obj.created_by_player_key == current_user_key)); // clear list
